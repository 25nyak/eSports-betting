...
class TeamA0 extends Component {
  constructor(){
    super();
    this.state={
      web3: '',
  ...
  getAmount(web3){
    ...
    Betting.deployed().then((instance) => {
      ...
      return BettingInstance.AmountOne.call({from: accounts[9]})
    }).then((result) => {
      this.setState({
  ...
  getName(web3){
    ...
    Betting.deployed().then((instance) => {
      ...
      return BettingInstance.getName1.call({from: accounts[9]})
    }).then((result) => {
      this.setState({
        teamname : result.c
      })
  ...
  Bet(){
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
        Betting.deployed().then((instance) => {
          ...
          return BettingInstance.bet(1, {from: accounts[9],
  ...
  MakeWin(){
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
        Betting.deployed().then((instance) => {
          ...
          return BettingInstance.distributePrizes(1, {from: accounts[9]})
        }).catch(() => {
