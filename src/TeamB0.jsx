...
import './App.css';
import equal from "./contracts/StringUtils.json";



class TeamB0 extends Component {
  constructor(){
    super();
    this.state={
      web3: '',
  ...
  getAmount(web3){
   ...
   getName(web3){
    ...
    Betting.deployed().then((instance) => {
      ...
      return BettingInstance.getName2.call({from: accounts[0]})
    }).then((result) => {
      this.setState({
        teamname : result
      })
  ...
  Results(){
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
		...
		var won=BettingInstance.getGameWinner()
		var team2=BettingInstance.getName2()
